    ...
    if ((last_bad_switches != switches_states) || (last_bad_pots != bad_pots)) {
      ...
      for (int i=0; i<NUM_SWITCHES; ++i) {
        if (SWITCH_WARNING_ALLOWED(i) && !(g_model.switchWarningEnable & (1<<i))) {
          ...
          if (attr) {
            if (++numWarnings < 6) {
              putsSwitches(x, y, SWSRC_FIRST_SWITCH+i*3, attr);
              x += 20;
            }
          ...
          if (!(g_model.potsWarnEnabled & (1 << i))) {
            if (abs(g_model.potsWarnPosition[i] - GET_LOWRES_POT_POSITION(i)) > 1) {
              ...
              switch (i) {
                ...
                case 2: 
#if defined(COLORLCD)
                  strcat(s, g_model.potsWarnPosition[i] > GET_LOWRES_POT_POSITION(i) ? "\176" : "\177");
#else
                ...
                case 4:
#if defined(COLORLCD)
                  strcat(s, g_model.potsWarnPosition[i] > GET_LOWRES_POT_POSITION(i) ? "\300" : "\301");
#else
