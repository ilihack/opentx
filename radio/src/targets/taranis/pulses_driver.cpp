...
static void extmoduleSbusStart( void ) ;
static void extmoduleSbusStop( void ) ;
...
void init_sbus(uint32_t port)
{
  if (port == EXTERNAL_MODULE)
    extmoduleSbusStart();
}

void disable_sbus(uint32_t port)
{
  if (port == EXTERNAL_MODULE)
    extmoduleSbusStop();
}
...
static void extmoduleSbusStart()
{
  EXTERNAL_MODULE_ON();
